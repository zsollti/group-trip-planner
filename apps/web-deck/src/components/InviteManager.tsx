...
/** The join URL a token resolves to, on this app's origin. */
function joinUrl(token: string): string {
  return `${window.location.origin}/join/${token}`;
}

/** Status chip text for a link. */
...
/**
 * Deck-paradigm invite surface: a ⌘K-summoned command dialog. Explains the
 * global-vs-personal distinction (FR-13) at the point of creation, then lists
 * existing links with copy/disable. Roles are limited to those strictly below
 * the caller's own — the same rule the API enforces.
 */
export function InviteManager({
  ...
  return (
    <div className="deck__palette-backdrop" role="presentation" onClick={onClose}>
      ...
      >
        <p className="deck__eyebrow">Invite</p>
        <h2 className="deck__dialog-title">Invite people to this trip</h2>

        <form onSubmit={onCreate} noValidate>
          <fieldset className="deck__radio-group">
            <legend className="deck__field-label">Link type</legend>
            <label className="deck__radio">
              <input
                type="radio"
                name="type"
                checked={type === "GLOBAL"}
              ...
              <span>
                ...
                join. One per role; disable it anytime (people who already joined
                stay).
              </span>
            </label>
            <label className="deck__radio">
              ...
              <span>
                ...
                optionally emailed. It's used up after the first join.
              </span>
            </label>
          </fieldset>
          ...
          {type === "PERSONAL" ? (
            <Field
              ...
              hint="We'll email the link. The link stays unbound — anyone who receives it can use it."
            >
              <Input
                id="invite-email"
                type="email"
        ...
        <div className="deck__invite-list">
          <p className="deck__eyebrow">Existing links</p>
          {invites.isPending ? (
            <p className="deck__muted">Loading links…</p>
          ) : invites.isError ? (
            <p className="deck__form-error" role="alert">
              Couldn't load invite links.
            </p>
          ) : invites.data.length === 0 ? (
            <p className="deck__muted">No links yet. Create one above.</p>
          ) : (
            <ul className="deck__invite-items">
              {invites.data.map((link) => {
                const active = linkStatus(link) === "Active";
                return (
                  <li key={link.id} className="deck__invite-item">
                    <div>
