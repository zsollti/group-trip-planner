...
/**
 * Feed-paradigm crew sheet: a slide-up roster with per-member role controls
 * gated by the `canActOn` strictly-lower rule (a Co-organizer sees no controls
 * on the Owner or a peer). The Owner can transfer ownership (with a confirm);
  ...
  return (
    <div className="feed__sheet-backdrop" role="presentation" onClick={onClose}>
      ...
      >
        ...
        <p className="feed__eyebrow">Crew</p>
        <h2 className="feed__title">Members &amp; roles</h2>

        {members.isPending ? (
          <p className="feed__muted">Loading crew…</p>
        ) : members.isError ? (
          ...
          <>
            <ul className="feed__invite-items">
              {members.data.members.map((m) => {
                ...
                return (
                  <li key={m.userId} className="feed__invite-item">
                    <div>
                      <strong>{m.displayName}</strong>
                      {isSelf ? (
                        <span className="feed__muted"> (you)</span>
                      ) : null}{" "}
                      <span className="feed__muted">{ROLE_LABEL[m.role]}</span>
                    </div>
            ...
            {canManage && members.data.blocked.length > 0 ? (
              <div className="feed__invite-list">
                <p className="feed__eyebrow">Blocked</p>
                <ul className="feed__invite-items">
                  {members.data.blocked.map((b) => (
                    <li key={b.userId} className="feed__invite-item">
                      <div>
